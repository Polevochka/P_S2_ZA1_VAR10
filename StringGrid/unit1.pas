...
var
  i,j: integer;
  width_one, height_one: integer; // ширина и высота одной ячейки таблицы
...
begin
  ...
  // 'tab.ColCount' , теперь же достаточно просто 'ColCount'
  with tab do
  begin

    {Устанавливаем одинаковую размер для всех ячеек, ячейки - КВАДРАТНЫЕ}
    // Сначала находим сколько пикселей на одну ячейчку
    width_one:= Width div n;   // ШИРИНА одной ячейки
    height_one:= Height div n;  // ВЫСОТА одной ячейки
    ...
    begin
      ...
      ColWidths[i] := width_one;   // для каждого столбца одна и та же ширина
    end;
